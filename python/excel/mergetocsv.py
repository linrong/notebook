...
def getdatabyexcel(excelpath):
    ...
    if len(excelfilelist) > 1:
        ...
        df2.to_excel(os.path.join(sys.path[0], '汇总物料' + '_' + str(int(time.time())) + '.xlsx'), sheet_name = 'Sheet1', index = False)
        example.signal.emit('已处理完成，保存地址为:' + sys.path[0])
...
class MainForm(QMainWindow, Ui_MainWindow):
    ...
    def getfolderpath(self):
        ...
        dir_choose = QFileDialog.getExistingDirectory(self, "选取文件夹", sys.path[0])
        if dir_choose == "":
